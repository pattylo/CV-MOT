...
vector<Match> hungarian::solution(vector<cv::Mat> measured, vector<MOT> previous )
{
    id_match.clear();
    cv::Point temp;
...
void hungarian::cost_generate(vector<cv::Point> detected, vector<cv::Point> previous)
{
...
void hungarian::find_min(double &minval)
{
    for (int r = 0; r < cost.rows(); r++)
        for (int c = 0; c < cost.cols(); c++)
...
void hungarian::find_star_in_col(int col, int &row)
{
    row = -1;
    for (int i = 0; i < cost.rows(); i++)
        if (mask(i, col) == 1)
